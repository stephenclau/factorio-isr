...
class RconClient:
    ...
    async def get_seed(self) -> str:
        """Get map seed."""
        try:
            response = await self.execute('/sc rcon.print(game.surfaces["nauvis"].map_gen_settings.seed)')
            return response.strip() if response else "Unknown"
        except Exception as e:
            logger.warning("get_seed_failed", error=str(e))
            raise

    async def get_evolution_factor(self) -> float:
        """Get enemy evolution factor for primary surface."""
        try:
            lua = (
                "/c "
                "local f = game.forces['enemy']; "
                "local evo = f.get_evolution_factor(game.surfaces['nauvis']); "
                "rcon.print(string.format('%.4f', evo))"
            )
            response = await self.execute(lua)
            return float(response.strip()) if response and response.strip() else 0.0
        except Exception as e:
            logger.warning("get_evolution_factor_failed", error=str(e))
            raise
